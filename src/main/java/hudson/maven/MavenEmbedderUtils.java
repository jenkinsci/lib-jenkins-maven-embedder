...
public class MavenEmbedderUtils {
    ...
    public static ClassRealm buildClassRealm(File mavenHome, ClassWorld world, ClassLoader parentClassLoader)
        ...
        AntClassLoader antClassLoader =
                new AntClassLoader(Thread.currentThread().getContextClassLoader(), false);
        ...
        ClassRealm classRealm =
                new ClassRealm(world, "plexus.core", parentClassLoader == null ? antClassLoader : parentClassLoader);
    ...
    public static PlexusContainer buildPlexusContainer(File mavenHome, MavenRequest mavenRequest)
            throws MavenEmbedderException {
        ClassWorld world = new ClassWorld("plexus.core", Thread.currentThread().getContextClassLoader());

        ClassRealm classRealm = MavenEmbedderUtils.buildClassRealm(
                mavenHome, world, Thread.currentThread().getContextClassLoader());
     ...
     */
    @SuppressFBWarnings("DP_CREATE_CLASSLOADER_INSIDE_DO_PRIVILEGED")
    public static PlexusContainer buildPlexusContainer(
            ClassLoader mavenClassLoader, ClassLoader parent, MavenRequest mavenRequest) throws MavenEmbedderException {
        DefaultContainerConfiguration conf = new DefaultContainerConfiguration();
        ...
        classRealm.setParentRealm(new ClassRealm(
                classWorld,
                "maven-parent",
    ...
    private static PlexusContainer buildPlexusContainer(
            MavenRequest mavenRequest, ContainerConfiguration containerConfiguration) throws MavenEmbedderException {
        try {
    ...
    /*package*/ static MavenInformation getMavenVersion(
            @Nonnull File mavenHome, @CheckForNull MavenEmbedderCallable preopertiesPreloadHook)
            throws MavenEmbedderException {
        ...
        try (ClassRealm realm = buildClassRealm(mavenHome, null, null)) {
            ...
            // TODO is this really intending to use findResource rather than getResource? Cf.
            // https://github.com/sonatype/plexus-classworlds/pull/8
    ...
    public static boolean isAtLeastMavenVersion(File mavenHome, String version) throws MavenEmbedderException {
        ComparableVersion found =
                new ComparableVersion(getMavenVersion(mavenHome).getVersion());
